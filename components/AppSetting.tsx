...
export default function AppSetting({ isOpen, onClose }: AppSettingProps) {
  ...
  const NotificationToggle = ({ label, desc, enabled, onToggle }: { label: string, desc: string, enabled: boolean, onToggle: () => void }) => (
    <div className="flex items-center justify-between py-4 border-b border-slate-50/10 last:border-0">
      <div className="flex flex-col gap-0.5">
        <span className="text-[15px] font-bold text-text-main">{t(label)}</span>
        <span className="text-[12px] font-medium text-text-muted">{t(desc)}</span>
      </div>
      <button 
        onClick={onToggle}
  ...
  const SubMenuLink = ({ label, desc, onClick }: { label: string, desc: string, onClick: () => void }) => (
    <button 
      onClick={onClick}
      className="flex items-center justify-between py-4 border-b border-slate-50/10 hover:bg-black/5 transition-colors px-2 -mx-2 rounded-xl group cursor-pointer"
    >
      <div className="flex flex-col items-start gap-0.5">
        <span className="text-[15px] font-bold text-text-main group-hover:text-[var(--primary)] transition-colors">{t(label)}</span>
        <span className="text-[12px] font-medium text-text-muted">{t(desc)}</span>
      </div>
  ...
  return (
    ...
    >
      ...
      <div 
        className="relative flex h-[80vh] w-full max-w-[900px] flex-col rounded-[24px] bg-surface-alt p-6 shadow-2xl sm:p-8 animate-fade-in border border-white/10"
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden rounded-[20px] bg-surface shadow-sm ring-1 ring-border-custom/10">
          
          {/* 좌측 네비게이션 탭 */}
          <aside className="w-[200px] bg-surface-alt/50 border-r border-slate-100/10 p-4">
                ...
                return (
                  <button
                    ...
                    className={`cursor-pointer px-4 py-3 text-center text-[14px] font-semibold transition-all ${
                      isActive
                        ? 'rounded-[14px] bg-surface text-[var(--primary)] shadow-sm ring-1 ring-slate-200/20' // 선택된 탭 강조
                        : 'text-text-muted hover:bg-black/5 hover:text-[var(--primary)] rounded-[14px]' // 호버 시 색상 변경 추가
                    }`}
                  >
                    {t(tab)}
                  </button>
          ...
          <main className="flex-1 p-8 overflow-y-auto">
            <h3 className="text-[20px] font-bold text-text-main mb-6">
            ...
            <div className="text-[14px] text-text-muted">
              {activeTab === 'Language' ? (
                <div className="flex flex-col gap-6">
                   <p className="text-text-main font-medium">{t('Select your preferred language')}</p>
                ...
                <div className="flex flex-col gap-8 animate-fade-in">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[15px] font-bold text-text-main">{t('Primary Color')}</span>
                      <span className="text-[12px] font-medium text-text-muted">{t('Choose your signature color for the app')}</span>
                    </div>
                      ...
                      {colorOptions.map((color) => (
                        <button
                          ...
                          className={`h-8 w-8 rounded-full border-2 transition-all cursor-pointer ${
                            primaryColor === color ? 'border-text-main scale-110 shadow-md' : 'border-transparent hover:scale-110'
                          }`}
